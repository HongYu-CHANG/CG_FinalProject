...
uniform sampler2D  rampTexture;           // 存放dissvole纹理
...
void main() {
	...
	vec4 vNoise = texture(rampTexture, texcoord);
	
    if(vNoise.r > dissolveFactor){
		discard;         // 如果小于阈值，就抛弃该片断	
	}
	
	else if(vNoise.r > dissolveFactor - 0.035f){
		outColor.rgb += vec3(255, 0, 0);
	}
	else if(vNoise.r > dissolveFactor - 0.05f){
		outColor.rgb += vec3(255.0, 0.0, 255.0);
	}
	else if(vNoise.r > dissolveFactor - 0.075f){
